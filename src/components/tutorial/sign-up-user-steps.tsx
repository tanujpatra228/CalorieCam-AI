...
export default function SignUpUserSteps() {
  const vercelEnv = config.app.vercelEnv;
  const vercelUrl = config.app.vercelUrl;
  const vercelProjectProductionUrl = config.app.vercelProjectProductionUrl;
            ...
            {vercelProjectProductionUrl && (
              <>
                ...
                <li>
                  -{" "}
                  <span className="relative rounded bg-muted px-[0.3rem] py-[0.2rem] font-mono text-xs font-medium text-secondary-foreground border">
                    {`https://${vercelProjectProductionUrl.replace(".vercel.app", "")}-*-[vercel-team-url].vercel.app/**`}
                  </span>{" "}
